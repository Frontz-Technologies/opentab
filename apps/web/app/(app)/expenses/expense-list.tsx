          ...
          {filtered.map((expense) => (
            ...
            >
              <div className="flex items-start justify-between mb-1">
                <span className="font-mono text-sm text-on-surface">{expense.expenseNumber}</span>
                <span className="font-label text-lg font-bold text-on-surface">{expense.currencyCode} {expense.total}</span>
              </div>
              <p className="text-sm text-on-surface mb-2">{expense.contactName || expense.description || "\u2014"}</p>
              <div className="flex items-center justify-between">
                <span className="text-xs text-on-surface-variant">{expense.expenseDate}</span>
                <Badge
                  className={statusColors[expense.status] ?? ""}
                  variant="outline"
                >
