...
export default async function DashboardPage() {
  ...
  return (
    <>
      <TopBar breadcrumbs={[{ label: "Dashboard" }]} userName={session.user.name} userEmail={session.user.email} />
      <main className="px-8 py-8 max-w-7xl mx-auto">
        <div className="mb-8">
          <h2 className="font-headline text-4xl font-extrabold text-on-surface tracking-tight mb-2">Dashboard</h2>
          <p className="text-on-surface-variant">Welcome to OpenTab, {session.user.name.split(" ")[0]}</p>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { label: "Revenue", icon: "trending_up" },
                { label: "Outstanding", icon: "schedule" },
                { label: "Expenses", icon: "payments" },
              ].map((kpi) => (
            ...
            <div className="bg-surface-container-low rounded-2xl p-8 border border-outline-variant/10 flex flex-col items-center justify-center min-h-[200px]">
              ...
              <p className="text-on-surface-variant text-sm mb-4">Create your first invoice to get started</p>
