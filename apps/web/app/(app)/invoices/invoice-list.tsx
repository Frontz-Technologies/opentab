          ...
          {filtered.map((invoice) => (
            ...
            >
              <div className="flex items-start justify-between mb-1">
                <span className="font-mono text-sm text-on-surface">{invoice.invoiceNumber}</span>
                <span className="font-label text-lg font-bold text-on-surface">{invoice.currencyCode} {invoice.total}</span>
              </div>
              <p className="text-sm text-on-surface mb-2">{invoice.contactName}</p>
              <div className="flex items-center justify-between">
                <span className="text-xs text-on-surface-variant">{invoice.issueDate}</span>
                <Badge className={getStatusColor(invoice)} variant="outline">{getStatusLabel(invoice, t)}</Badge>
              </div>
            </Link>
          ))}
        </div>
